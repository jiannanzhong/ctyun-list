...
$(function () {
    ...
    $('#table-dir').on('click', '.renameBtn', function () {
        var firstTD = $(this).parent().parent().children()[0];
        var text = $(firstTD).find('a').text().trim();
        var id = $(firstTD).find('label').attr('for');

        $('#rename-dir-name').val(text);
        $('#rename-dir-id').val(id);
        $('#showRenameViewBtn').click();
        var tempTimer = setInterval(function () {
            $('#rename-dir-name')[0].focus();
            clearInterval(tempTimer);
        }, 600);
        ...
        success: function (data, status, xhr) {
            ...
            for (var i = 0; i < userDirs.length; i++) {
                ...
                if (userDirs[i].is_dir === 1) {
                    ...
                    dirName = '<i class="fa fa-file-text-o" aria-hidden="true"></i>&nbsp;<a target="_blank" download="' + userDirs[i].name + '" href="' +
                        data['prjPath'] +
                ...
                $(tbody).append(
                    $('<tr></tr>').append(
                        ...
                        $('<td></td>').attr('class', 'renameTD').append(
                            $('<button class="btn btn-info renameBtn" type="button">重命名</button>')
                        )
                    )
                );
            }
